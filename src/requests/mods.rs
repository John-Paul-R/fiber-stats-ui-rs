...
#[track_caller]
fn log<T, E, Ctx: Display>(fn_name: &str, result: &Result<T, Error<E>>, ctx: Ctx) {
...
pub async fn get_stats(mod_id: String) -> Option<ModStatsResponse> {
    ...
    let result = mods_api::api_v10_mods_id_stats_get(
        &REQUEST_CONFIG,
        id_str)
        .await;
...
pub async fn get_mod(mod_id: String) -> Option<ModResponse> {
    let result = mods_api::api_v10_mods_id_get(
        &REQUEST_CONFIG,
        mod_id.as_str())
        .await
        ;

    // there can be an errors at deserialization time that we are throwing away here...
